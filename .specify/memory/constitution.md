...
- PostgreSQL is the queue source of truth: claims lock `queue_tenant_state` and jobs with `FOR UPDATE SKIP LOCKED`, use `last_dispatched_at NULLS FIRST, account_id` tenant order and `priority DESC, available_at, created_at, id` job order, a lease plus monotonic fencing token, heartbeat, bounded retry/backoff, reaper recovery, and tenant/global concurrency limits. Transactional outbox rows are committed with state changes and use their own short lease/fencing/heartbeat recovery; `LISTEN/NOTIFY` may only be a wake-up hint and may never be the delivery guarantee. Redis and Kafka are not required for this boundary.
