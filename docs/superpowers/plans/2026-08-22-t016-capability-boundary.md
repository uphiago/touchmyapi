...
- [ ] **Step 1: Add the closed-surface assertion**
...
- [ ] **Step 2: Add the live-grant RED test**
...
- [ ] **Step 1: Implement the opaque context**

Export `RuntimeRole` and `TenantContext<R>` from the package root. Give the context only `role` and `account.readCurrent()`. Keep the account identifier in the private active-executor record; callers cannot pass it to a repository operation.

- [ ] **Step 2: Implement the private executor**
...
- [ ] **Step 3: Implement the account repository**
...
- [ ] **Step 1: Update task status only after review approval**
