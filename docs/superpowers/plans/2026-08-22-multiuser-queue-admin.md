...
**Goal:** Add explicit account membership, a fenced PostgreSQL queue/outbox, and an isolated staff admin plane while preserving the existing `user` identity authority and Constitution 1.1.3.

**Architecture:** The existing `user` table remains the single global Google identity authority. `account_membership(account_id,user_id)` is the tenant authorization boundary; `session.account_id` is the one active account and switches rotate the session. PostgreSQL is queue source of truth with singleton `queue_global_state` capacity locking, exact tenant fairness, global→tenant→job `FOR UPDATE` lock order for every job/counter mutation, `SKIP LOCKED`, leases, monotonic fencing, reaper/backoff, and a least-privilege `queue_control` function boundary. Tenant enqueue uses closed typed `packages/db/src/queue.ts` under `api_rls`/`app.tenant`; worker operations use typed `packages/db/src/queue-control.ts`, whose dedicated connector has only `EXECUTE` on fixed-signature worker queue/outbox functions and zero table grants. Standalone outbox locks never touch global/job; admin queue operations use separate JIT-gated functions. Admin has separate staff identity/OIDC, WebAuthn MFA, cookies, JIT support grants, and dual approval.
...
`LISTEN/NOTIFY` is only a wake-up hint. Never use Redis/Kafka, a second customer identity table, email auto-linking, URL bearer tokens, owner/BYPASSRLS runtime access, impersonation, arbitrary SQL, secret/raw-evidence admin access, or billing writes. Tenant enqueue uses only closed typed `packages/db/src/queue.ts` under `api_rls`/`app.tenant` after membership/policy/entitlement checks; worker queue/outbox code uses `packages/db/src/queue-control.ts` with function-only `queue_connector`; admin queue actions use separate JIT-gated `packages/db/src/admin-queue.ts`/`admin_queue_connector`. Queue application code never uses unsafe SQL or direct queue table grants.
...
| Queue/outbox | `packages/db/schema/{queue,execution}.ts`, `packages/db/src/{queue,queue-bootstrap,queue-control}.ts`, `apps/worker-control/src/{scheduler,reaper,outbox-dispatcher}.ts` | Tenant-policy-gated enqueue, least-privilege queue bootstrap/function boundary, global singleton capacity, global→tenant→job operations, standalone outbox locks, lease, fencing, heartbeat, recovery, exact fairness, transactional outbox |
| Admin | `packages/db/schema/admin.ts`, `packages/db/src/{admin-grants,admin-queue}.ts`, `apps/api/src/routes/admin-{grants,support,billing,queue}.ts`, `apps/admin/src` | Staff bootstrap/OIDC/WebAuthn/recovery, grants/approvals, separate JIT queue functions, safe queue/billing read API |
...
- Create: `packages/db/src/queue.ts` (closed `api_rls` tenant enqueue)
...
- [ ] **RED:** Test closed queue/outbox schemas, singleton `queue_global_state`, `queue_tenant_state`, global/tenant counters, statuses, fencing, exact standalone outbox signatures/locks, max attempts, partial active index statuses, `queue_control` role attributes, `FORCE RLS` policies, exact operational column grants, queue-connector zero table grants/no enqueue, admin connector separation, and bootstrap upserts. Run `bun run test:contract -- queue && RUN_DB_TESTS=1 DATABASE_URL=postgres://touchmyapi_dev:touchmyapi_dev@127.0.0.1:5433/touchmyapi_multiuser_test bun run test:integration --maxWorkers=1 -- queue-schema`; expected `FAIL`.
- [ ] **GREEN:** Define singleton `queue_global_state(id='global',running_count,concurrency_limit)`, `queue_tenant_state(account_id,last_dispatched_at,running_count,concurrency_limit)`, job/outbox operational fields, and partial unique `(account_id,normalized_target_key)` for `queued/stale_recovered/running`. Generate and inspect the next migration, create `queue_control` as `NOLOGIN NOSUPERUSER NOBYPASSRLS NOINHERIT`, `queue_connector` with only worker queue/outbox `EXECUTE`, and separate `admin_queue_connector` with only JIT-gated admin function `EXECUTE`; enable `FORCE RLS`, add singleton `id='global'` and definer-only cross-tenant policies, grant only listed operational columns, create `packages/db/src/queue-bootstrap.ts` to upsert the singleton, and backfill the singleton plus one tenant row for every active account. Upsert the tenant row in `apps/api/src/routes/account.ts` and `packages/db/src/auth-bootstrap.ts`; reconciler may derive missing rows only from operational job/outbox account IDs; avoid advisory locks/`SERIALIZABLE` retries. Run `bun run test:contract -- queue && RUN_DB_TESTS=1 DATABASE_URL=postgres://touchmyapi_dev:touchmyapi_dev@127.0.0.1:5433/touchmyapi_multiuser_test bun run test:integration --maxWorkers=1 -- queue-schema && bun run typecheck`; expected `PASS`.
...
**Files:**
- Create: `packages/db/src/queue.ts` (typed tenant enqueue; no unsafe SQL)
- Create: `packages/db/src/queue-control.ts` (typed worker calls only; no unsafe SQL)
...
- [ ] **GREEN:** Implement atomic worker reaper and bounded retry/jitter through `queue_control`: lock global first, then tenants `ORDER BY account_id FOR UPDATE SKIP LOCKED`, then jobs `ORDER BY id FOR UPDATE SKIP LOCKED`; decrement both counters, apply stale recovery/backoff or exhausted failure, and never widen scope/policy or reset fencing. Implement standalone `outbox_reap(int,timestamptz)` with `account_id,id` locks, no global/job touch, and the same retry/exhaustion semantics. Run `RUN_DB_TESTS=1 DATABASE_URL=postgres://touchmyapi_dev:touchmyapi_dev@127.0.0.1:5433/touchmyapi_multiuser_test bun run test:integration --maxWorkers=1 -- queue-recovery && bun run typecheck`; expected `PASS`.
...
- [ ] **GREEN:** Implement the global singleton plus `queue_tenant_state` reconciler through `queue_control`: lock global first, then tenants `ORDER BY account_id FOR UPDATE SKIP LOCKED`, then jobs `ORDER BY id FOR UPDATE SKIP LOCKED`; derive both running counts, create missing tenant rows only from operational job/outbox account IDs, repair drift, fail closed without discarding queued jobs, and verify account-create/auth-bootstrap upserts provide active-account completeness without account/membership SELECT. Run `bun run test:unit -- fair-scheduler && RUN_DB_TESTS=1 DATABASE_URL=postgres://touchmyapi_dev:touchmyapi_dev@127.0.0.1:5433/touchmyapi_multiuser_test bun run test:integration --maxWorkers=1 -- queue-reconcile && bun run typecheck`; expected `PASS`.
...
- [ ] **GREEN:** Implement typed standalone calls `outbox_claim(text,int,timestamptz)`, `outbox_heartbeat(uuid,uuid,text,bigint,timestamptz)`, `outbox_ack(uuid,uuid,text,bigint,timestamptz)`, `outbox_fail(uuid,uuid,text,bigint,text,timestamptz)`, and `outbox_reap(int,timestamptz)` through `queue_control` with `available_at,account_id,id` claim locks and `account_id,id` other locks; exact transitions `pending`→`processing`→`processed`, current-token failure/backoff until `max_attempts`, terminal `failed` with redacted `last_error`/`failed_at`, alert and audit, and expired-processing reaper using the same exhaustion path. Keep job-aggregate insertion in the original global→tenant→job transaction; standalone outbox functions never touch global/job. Keep `pg_notify` optional and never authoritative. Run `RUN_DB_TESTS=1 DATABASE_URL=postgres://touchmyapi_dev:touchmyapi_dev@127.0.0.1:5433/touchmyapi_multiuser_test bun run test:integration --maxWorkers=1 -- outbox && bun run typecheck`; expected `PASS`.
...
- [ ] **GREEN:** Wire existing T027/T029 to typed queue/enqueue/control functions delivered by T081–T086; do not duplicate queue semantics in T027/T029. Record FR-024/FR-025/SC-004/SC-014/SC-015 evidence, including global→tenant→job ordering for job/counter terminal/reaper/reconcile paths, standalone outbox-only locks, and stale-fence no-op, and leave T022 unchecked until this gate. Run `RUN_DB_TESTS=1 DATABASE_URL=postgres://touchmyapi_dev:touchmyapi_dev@127.0.0.1:5433/touchmyapi_multiuser_test bun run test:integration --maxWorkers=1 -- queue-recovery && RUN_DB_TESTS=1 DATABASE_URL=postgres://touchmyapi_dev:touchmyapi_dev@127.0.0.1:5433/touchmyapi_multiuser_test bun run test:integration --maxWorkers=1 -- assessment-concurrency && bun run test:contract && git diff --check`; expected `PASS`.
...
- Create: `packages/db/src/admin-queue.ts`
- Modify: `packages/policy/src/engine.ts`
- Create: `tests/contract/admin-queue-policy.test.ts`

- [ ] **RED:** Test only metadata/status inspect, cancel, requeue, and `POST /admin/accounts/:accountId/reaper/run` with a valid account-bound `queue_reaper` grant and bounded `maxJobs <= 100`; assert the admin connector cannot use `queue_connector` or table grants, requeue clears lease but never resets fencing; scope/action/target changes, global reaper, and arbitrary dispatch deny. Run `bun run test:contract -- admin-queue-policy`; expected `FAIL`.
- [ ] **GREEN:** Route each admin operation through policy, account-bound support grant, and separate `packages/db/src/admin-queue.ts` fixed functions via `admin_queue_connector`; preserve queue module semantics and fencing, and never expose a system-wide reaper endpoint. Run `bun run test:contract -- admin-queue-policy && bun run typecheck`; expected `PASS`.
- [ ] **Commit:** `git add apps/api/src/routes/admin-queue.ts packages/db/src/admin-queue.ts packages/policy/src/engine.ts tests/contract/admin-queue-policy.test.ts && git commit -m "admin: gate queue operations by policy"`.
