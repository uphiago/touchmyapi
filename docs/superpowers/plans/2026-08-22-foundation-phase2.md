...
| DB/RLS | `packages/db/package.json`, `packages/db/schema/{common,identity,assessment,catalog,execution,billing,audit,index}.ts`, `packages/db/migrations/{0000_foundation,0001_rls_roles}.sql`, `packages/db/src/{index,tenant-session,auth-bootstrap,audit}.ts`, `packages/db/test/*.test.ts`, `tests/isolation/rls.test.ts` | Schema completo, roles, policies, funções bootstrap, transação tenant e cadeia de auditoria |
