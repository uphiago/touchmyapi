...
- [ ] Inspect `packages/db/drizzle/meta/_journal.json` and highest migration. Generate the next migration with `bunx drizzle-kit generate --config=drizzle.config.ts --name multiuser_membership` against the fresh `_test` URL; inspect SQL before applying it.
