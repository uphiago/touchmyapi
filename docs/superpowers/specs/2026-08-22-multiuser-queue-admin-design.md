...
`queue_tenant_state(account_id,last_dispatched_at,running_count,concurrency_limit)` is the exact fairness state. Account creation transactionally upserts it, migration backfills one row for every active account, and the reconciler creates missing rows and repairs drift. If the row is missing or inconsistent, enqueue/claim fails closed while leaving jobs queued for reconciliation; no job is dropped or stranded.
