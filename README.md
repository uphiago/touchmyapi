...
T020 and T021 are accepted. T020 adds the dependency-injected Hono API boundary with exact CORS, server-owned request IDs, pre-handler redaction-safe audit gating, stable error envelopes, and fail-closed audit configuration. T021 adds a Google-only, fakeable OAuth Authorization Code + PKCE boundary with encrypted transient state, hash-only sessions, rotation, revocation, and secure cookies. Specification and adversarial quality reviews passed. The multi-user, PostgreSQL queue/outbox, and separate admin control-plane work in T071–T094 is design-only and remains unchecked.
