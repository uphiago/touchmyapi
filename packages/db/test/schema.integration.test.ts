...
schemaDescribe("PostgreSQL foundation schema", () => {
  ...
  it("enforces tenant ownership columns and required unique constraints", async () => {
    ...
    const expectedUnique = [
      ...
      "finding:finding_assessment_source_unique:account_id,assessment_id,source_key",
      "job:job_account_id_id_unique:account_id,id",
      "job:job_dedupe_key_unique:dedupe_key",
      "notification:notification_account_event_unique:account_id,event_key",
      ...
      "report:report_assessment_kind_unique:account_id,assessment_id,kind",
      "runner_execution:runner_execution_account_id_id_unique:account_id,id",
      "runner_execution:runner_execution_job_fence_unique:account_id,job_id,fencing_token",
  ...
  it("matches critical defaults, primary keys, nullability, and checks", async () => {
    ...
    expect(actualChecks).toEqual(
      new Map([
        ...
        ["finding_source_key_bounded", "check lengthsource_key >= 1 and lengthsource_key <= 255"],
        ...
        ["notification_event_key_bounded", "check lengthevent_key >= 1 and lengthevent_key <= 255"],
        ...
        ["runner_execution_fencing_nonnegative", "check fencing_token >= 0"],
