...
Admin audit additionally records staff identity, capability grant, ticket reference, reason, approval count, TTL, outcome, `prev_event_hash`, and `event_hash` in an append-only per-account chain; `account_id IS NULL` is reserved for the separate system/bootstrap chain. Billing records remain read-only to admin; entitlement and credit changes still require verified Stripe webhooks.
