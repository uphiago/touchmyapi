...
Tenant enqueue is performed only by the closed typed `packages/db/src/queue.ts` API under `api_rls`/`app.tenant`, after membership, policy, and entitlement checks; it inserts the job and versioned redacted outbox payload atomically. The worker reaches operational queue state only through typed `packages/db/src/queue-control.ts` calls to fixed-signature `SECURITY DEFINER` functions owned by `queue_control`; the dedicated worker connector has only `EXECUTE` on worker queue/outbox functions and zero table grants. Completion, heartbeat, failure, and worker recovery lock global→identified tenant→identified job in one transaction. Customer cancellation uses the tenant API boundary; admin cancellation, requeue, and account-reaper use separate JIT-gated `packages/db/src/admin-queue.ts` functions through `admin_queue_connector`, never `queue_connector`. Reaper and reconciliation lock global, then tenants `ORDER BY account_id`, then jobs `ORDER BY id`; a stale fence is a no-op, including no counter change. Standalone outbox delivery uses `outbox_claim`, `outbox_heartbeat`, `outbox_ack`, `outbox_fail`, and `outbox_reap` with `available_at,account_id,id`/`account_id,id` locks and never touches global/job rows. The queue control boundary cannot read business payloads, credentials, evidence, or reports and has no owner, `BYPASSRLS`, or arbitrary SQL path.
