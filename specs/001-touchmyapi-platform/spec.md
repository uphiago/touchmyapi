...
- **SC-014**: Concurrent workers, global→tenant→job locks, `queued`/`stale_recovered` claims, stale leases, worker crashes, retry exhaustion, cancellation, exact `queue_tenant_state` fairness, tenant/global limits, missing/drifted state reconciliation, fail-closed queue behavior, and missed notifications leave no lost, stranded, or doubly completed job; fencing and reaper tests pass.
