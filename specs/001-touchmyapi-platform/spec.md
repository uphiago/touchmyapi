...
The control worker reaches PostgreSQL through typed `packages/db/src/queue-control.ts` calls to fixed-signature functions owned by a least-privilege `queue_control` role; its dedicated connector has only `EXECUTE` and zero table grants. Claims use `FOR UPDATE SKIP LOCKED`, a lease, and a fencing token. Heartbeats, completion, failure, cancellation, retry/backoff, timeout, reaper recovery, reconciliation, fair scheduling, tenant/global limits, and a transactional outbox make worker restarts and missed notifications safe. Staff use a separate admin app/API/origin, identity, cookies, and mandatory MFA. Tenant access is granted just in time with a reason, ticket, TTL, and approval; break-glass requires two approvers. Admin operations are policy-aware and expose neither secrets nor raw evidence.

**Independent Test**: Concurrent workers claim distinct jobs, connector isolation proves no direct table/business-data access, stale workers cannot complete after lease loss, completion/failure/cancel/reaper/reconcile use global→tenant→job locks with deterministic batch order and stale-fence no-op, reaper/backoff recovers abandoned jobs, outbox delivery survives a missed `NOTIFY`, and admin tests prove separate MFA sessions, TTL-bound capability grants, dual break-glass, no impersonation, no arbitrary SQL, no RLS bypass, read-only billing, and no secret/raw-evidence access.
...
- **FR-024**: System MUST use PostgreSQL as queue source of truth behind typed `packages/db/src/queue-control.ts` calls to fixed-signature `SECURITY DEFINER` functions owned by `queue_control` (`NOLOGIN NOSUPERUSER NOBYPASSRLS NOINHERIT`), while the dedicated queue connector has only `EXECUTE` and zero table grants. Direct privileges/RLS MUST be limited to `queue_global_state`, `queue_tenant_state`, and required operational metadata/state columns of `job`/`outbox_event`; business payloads, evidence, credentials, owner access, `BYPASSRLS`, and arbitrary SQL are forbidden. Claims MUST lock singleton `queue_global_state` `FOR UPDATE`, then tenant `FOR UPDATE SKIP LOCKED`, then job `FOR UPDATE SKIP LOCKED` (global→tenant→job); completion, heartbeat, failure, cancellation, requeue, reaper, and reconciliation MUST use the same global→tenant→job order in one transaction, with batch tenants ordered by `account_id` and jobs by `id`, and stale fencing a no-op. Claims accept `queued` or `stale_recovered`, set `running`, increment a monotonic fencing token, and enforce heartbeat, bounded retry/backoff, timeout, exact tenant fairness through `queue_tenant_state` for every active account, partial unique active target/account protection across `queued`/`stale_recovered`/`running`, and tenant/global limits; queue bootstrap upserts the singleton, account creation/auth bootstrap upserts its tenant row in the same transaction, migration backfills the singleton plus every active-account tenant row, reconciliation repairs missing/drifted counters, and missing state fails closed without dropping or stranding jobs; advisory hash locks and `SERIALIZABLE` retry ambiguity are forbidden; Redis and Kafka are not required.
- **FR-025**: System MUST commit transactional outbox events with state changes, claim outbox rows with short leases/fencing/heartbeat, recover expired processing rows, transition `pending`→`processing`→`processed`, retry current-token failures with redacted `last_error` and bounded backoff until `max_attempts`, then transition terminally to `failed` with `failed_at`, alert, and audit; processing is at-least-once idempotently and failed rows are never reclaimed; `LISTEN/NOTIFY` MAY wake a poller but MUST NOT be the delivery guarantee.
...
- **QueueControlBoundary**: `queue_control` definer role, `queue_connector` execute-only role, fixed-signature functions, queue-only RLS/operational columns, and typed `packages/db/src/queue-control.ts` API.
