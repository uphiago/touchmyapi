...
TouchMyAPI is an authorized security-assessment platform. Customers authenticate with GitHub Authorization Code + PKCE, select an account/workspace through explicit membership, create assessments against clearly scoped external targets (and, later, internal environments via a private agent), and run versioned playbooks with hard limits enforced by a policy engine. A durable PostgreSQL queue drives isolated runner sandboxes; claims lock singleton `queue_global_state` then `queue_tenant_state` then jobs, use `SKIP LOCKED`, exact fairness, leases, monotonic fencing, heartbeat, retry/backoff, reaper, tenant/global limits, and a transactional at-least-once outbox with terminal failed/alert/audit behavior. Tenant enqueue is a closed typed `packages/db/src/queue.ts` operation under `api_rls`/`app.tenant` after membership/policy/entitlement checks, while worker queue/outbox operations use typed `packages/db/src/queue-control.ts` fixed-signature functions with a connector that has only `EXECUTE` and zero table grants. Standalone outbox delivery locks `available_at,account_id,id` or `account_id,id` and never touches global/job rows; job-aggregate event insertion remains in the global→tenant→job transaction. Plan gating controls what results, evidence, PDFs, and JSON the user may see; Stripe webhooks are the only source of truth for entitlement; DeepSeek and Codex act as non-executor AI (planner/triage and report drafting, respectively). A separate MFA-protected admin control plane provides policy-aware account-scoped queue operations through JIT-gated admin functions without impersonation, RLS bypass, arbitrary SQL, secret/raw-evidence access, or billing writes. The build order is: foundation T010–T021, membership T071–T080, queue/outbox T081–T087, admin T088–T094, then existing assessment state tests T022 and later Stripe/playbook/verification/report/agent increments.

## Technical Context

**Language/Version**: Bun 1.x (API, control worker), TypeScript strict throughout; Vite + React 18/19 (web client)

**Primary Dependencies**: Hono on Bun, PostgreSQL driver (`postgres`/postgres.js for runtime queries under RLS; Drizzle ORM for schema/migrations), Stripe SDK + webhook signing, GitHub OAuth via Authorization Code + PKCE, Vitest (unit/contract/integration), React Router, zod for schema validation, motion (animations, per frontend design law)
...
**Constraints**: policy engine is final authority; browser/model/runner cannot escalate limits; RLS default-deny; tenant enqueue requires `api_rls`/`app.tenant` membership, policy, and entitlement checks; queue connector has only worker-function `EXECUTE`, no enqueue/admin function, and zero table grants; queue-control direct access is limited to exact operational queue state; standalone outbox never touches global/job rows; credentials never in logs/reports/models/frontend; webhook-only entitlement changes; AI non-executor; no bucket public; one run per target/account
...
- **Phase 2A account isolation and queue/admin amendment**: PASS - spec FR-022–FR-027, constitution v1.1.3, explicit membership/RLS, typed tenant enqueue, least-privilege PostgreSQL queue/outbox-control boundary, standalone fenced outbox, and separate MFA admin origin with no bypass capabilities.
