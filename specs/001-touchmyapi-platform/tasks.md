...
- [ ] T081 [P] [INFRA] Define queue/outbox contracts/schema and least-privilege bootstrap (`packages/contracts/src/queue.ts`, `packages/db/schema/queue.ts`, `packages/db/src/queue-bootstrap.ts`, next generated `packages/db/migrations/*.sql` after journal inspection): singleton `queue_global_state`, `queue_tenant_state`, eligible statuses, partial active index over `queued/stale_recovered/running`, outbox lease/heartbeat/fencing/max_attempts/last_error/failed_at, `queue_control`/`queue_connector` role boundary, backfill the singleton plus one tenant row for every active account, queue-bootstrap singleton upsert, transactional account-create/auth-bootstrap tenant upsert, and fail-closed behavior that leaves jobs queued when state is missing.
- [ ] T082 [INFRA] Implement enqueue/exact fair claim through typed `packages/db/src/queue-control.ts` (scheduler): dedicated connector has only `EXECUTE` on fixed-signature `SECURITY DEFINER` functions, zero table grants; global `FOR UPDATE` then tenant `FOR UPDATE SKIP LOCKED` order `last_dispatched_at NULLS FIRST,account_id`, then job `FOR UPDATE SKIP LOCKED` order `priority DESC,available_at,created_at,id`; atomic global/tenant counters/timestamp; postgres.js driver; no unsafe SQL, advisory hashes, or `SERIALIZABLE` retries; isolation tests prove no cross-tenant business-data access.
- [ ] T083 [INFRA] Implement heartbeat/ack/fail/cancel through `packages/db/src/queue-control.ts`: every transaction locks global→tenant→job, uses account/lease-owner/fencing predicates, stale fence is a no-op, terminal paths decrement both counters, never reset fencing, and admin requeue clears lease only.
...
- [ ] T085 [INFRA] Implement fairness reconciler (`apps/worker-control/src/{fair-scheduler,reconciler}.ts` via `packages/db/src/queue-control.ts`): singleton/global and exact `queue_tenant_state`, global/tenant caps, noisy/quiet proof, create missing rows for active accounts, lock global→tenants `ORDER BY account_id`→jobs `ORDER BY id`, repair both running-count drifts, fail closed when state is missing, and keep jobs queued rather than stranded; no deficit/fair score.
- [ ] T086 [INFRA] Implement transactional outbox through `packages/db/src/queue-control.ts` and dispatcher: claim `SKIP LOCKED`, short lease/heartbeat/fencing, max attempts, redacted last error, terminal failed/failed_at, alert/audit, expired processing recovery, at-least-once idempotent event key, polling with `NOTIFY` hint, and global→tenant→job order for job-aggregate mutations.
- [ ] T087 [US1] Integrate typed queue-control primitives into T024/T025/T027/T029 and gate (`tests/integration/{queue-recovery,assessment-concurrency}.test.ts`, review): T024 selects `stale_recovered`, T025 consumes active index, T027/T029 integrate without duplicate semantics, and completion/cancel/fail/reaper/reconcile preserve global→tenant→job order with stale-fence no-op; FR-024/025 and SC-004/014/015 evidence.
...
- [ ] T092 [US6] Implement policy-aware queue ops (`apps/api/src/routes/admin-queue.ts`, policy test): account-scoped inspect/cancel/requeue and `/admin/accounts/:accountId/reaper/run` only; require the account-bound support grant capability and bounded batch for reaper; requeue clears lease and never resets fencing; no scope/action/target changes, global reaper, or arbitrary dispatch.
