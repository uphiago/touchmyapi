...
- [ ] T029 [US1] Integrate `apps/worker-control/src/scheduler.ts` with INFRA T082–T086: lock `queue_global_state` then exact `queue_tenant_state` fairness claim, dispatch only current fencing token, heartbeat before half-lease, handle cancel/cleanup, and poll outbox; do not reimplement queue primitives here
...
- [ ] T081 [P] [INFRA] Define queue/outbox contracts/schema (`packages/contracts/src/queue.ts`, `packages/db/schema/queue.ts`, next generated migration): singleton `queue_global_state`, `queue_tenant_state`, eligible statuses, partial active index over `queued/stale_recovered/running`, outbox lease/heartbeat/fencing/max_attempts/last_error/failed_at, backfill the singleton plus one tenant row for every active account, queue-bootstrap singleton upsert, transactional account-create/auth-bootstrap tenant upsert, and fail-closed behavior that leaves jobs queued when state is missing.
- [ ] T082 [INFRA] Implement enqueue/exact fair claim (`packages/db/src/queue.ts`, scheduler): global `FOR UPDATE` then tenant `FOR UPDATE SKIP LOCKED` order `last_dispatched_at NULLS FIRST,account_id`, then job `FOR UPDATE SKIP LOCKED` order `priority DESC,available_at,created_at,id`; atomic global/tenant counters/timestamp; postgres.js driver; no advisory hashes or `SERIALIZABLE` retries.
...
- [ ] T084 [INFRA] Implement reaper/retry (`apps/worker-control/src/reaper.ts`, queue tests): `running→stale_recovered` with available_at/backoff and global/tenant counter decrement, next claim stale→running with token increment, exhausted→failed, cleanup.
- [ ] T085 [INFRA] Implement fairness reconciler (`apps/worker-control/src/{fair-scheduler,reconciler}.ts`): singleton/global and exact `queue_tenant_state`, global/tenant caps, noisy/quiet proof, create missing rows for active accounts, repair both running-count drifts, fail closed when state is missing, and keep jobs queued rather than stranded; no deficit/fair score.
- [ ] T086 [INFRA] Implement transactional outbox (`packages/db/src/outbox.ts`, dispatcher): claim `SKIP LOCKED`, short lease/heartbeat/fencing, max attempts, redacted last error, terminal failed/failed_at, alert/audit, expired processing recovery, at-least-once idempotent event key, polling with `NOTIFY` hint.
