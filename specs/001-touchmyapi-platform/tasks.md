...
- [x] T078a [P] Add a reproducible local stack path (`scripts/dev-local.ts`, `scripts/local-smoke.ts`, `dev:local`, `local:smoke`, `local:logs`) that starts loopback PostgreSQL, applies migrations, keeps API/Vite logs attached, and verifies `/health` plus the web shell while running.
...
- [ ] T081 [INFRA] Define queue/outbox contracts/schema and least-privilege bootstrap (`packages/contracts/src/queue.ts`, `packages/db/schema/queue.ts`, `packages/db/src/queue-bootstrap.ts`, next generated `packages/db/migrations/*.sql` after journal inspection): singleton `queue_global_state`, `queue_tenant_state`, eligible statuses, partial active index over `queued/stale_recovered/running`, exact outbox operational fields and standalone function signatures, `queue_control`/worker-only `queue_connector`/`admin_queue_connector` role boundary, `FORCE RLS` singleton and cross-tenant definer-only policies, exact operational column grants, backfill the singleton plus one tenant row for every active account, queue-bootstrap singleton upsert, transactional account-create/auth-bootstrap tenant upsert, and fail-closed behavior that leaves jobs queued when state is missing.
