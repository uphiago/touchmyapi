...
- [ ] T076 [US5] Complete the remaining account/membership/lifecycle boundary (`apps/api/src/memberships.ts`, `packages/db/migrations/0016_membership_lifecycle.sql`): the canonical list/invite/patch/remove routes, active-session checks, session revocation, role guard, audit chain, production PostgreSQL wiring, and customer UI are present and covered by T080; account deletion/data-elimination workflow and its final acceptance evidence remain open.
...
- [x] T099 [INFRA] Add the Barbarossa-pattern GitHub/OVH release (`.github/workflows/build-deploy.yml`, `scripts/{deploy-ovh,deploy-edge-ovh,smoke-remote}.sh`, `infra/edge/Caddyfile`, `docs/operations/ovh-deployment.md`): PR validation, manual/`v*` release only, SHA-pinned actions and images, minimal permissions, GitHub `production` environment, verified host key/strict SSH, host-provisioned `.env` preservation, serialized deploy, bounded image pruning, migration-before-cutover, tracked Caddy apex/app routing with validated edge-container recreation and previous-config fallback, remote smoke, and prior-SHA rollback metadata. The latest immutable production cutover was proven by Actions run `32690465770` at merge SHA `2580a6e98df56f74aaaeec1b10ede8bc0c06bb60`; R2 remains disabled at the account boundary and is not part of the current runtime.
- [x] T100 Run the console/admin/deploy checkpoint review (`docs/reviews/2026-08-23-console-admin-ovh-checkpoint.md`): customer/admin production builds, local browser captures at desktop/mobile, customer and admin smoke, workflow/Compose regression tests, full fast gates, and the separately migrated PostgreSQL integration/isolation suites recorded at that checkpoint. New database acceptance runs must use fresh isolated `_test` databases; see `docs/reviews/2026-08-24-total-review.md`. Mark only T095–T100 complete; T087 and T088–T094 remain governed by their full acceptance criteria.
...
- [x] T103 [US1/US5] Deliver the public landing and role-aware customer journey for owner/admin/operator/viewer/billing, named local persona workspaces, team lifecycle controls, explicit final queue review, and server-gated delivery matrix. The apex serves the landing and `app` serves the console; see `docs/product/user-journeys.md`.
